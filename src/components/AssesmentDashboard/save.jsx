// src/components/Dashboard/CareerDashboard.jsx
...
const CareerDashboard = () => {
  // Use context for results and loading state
  ...
  const getDefaultResults = () => ({
    analysis: { // Ensure structure matches expected data
      careerRecommendations: [],
      skillsAnalysis: { strengths: [], skillsToDevelop: [] },
      actionPlan: { // Changed from actionPlans
        immediateNextSteps: [],
        shortTermGoals: [],
        longTermRoadmap: [],
      },
      ...
      insights: { // Changed from personalInsights
        keyTakeaways: [],
        motivationalQuote: "",
      },
      resources: {
      ...
      recommendedSchools: [], // Added for completeness
      recommendedMentors: [], // Added for completeness
    }
  });
  ...
  const fetchResults = async () => {
    try {
      setIsLoading(true);
      if (!user) {
        // No throw here, allow guest view or specific handling
      ...
      if (!data || !data.results || !data.results.analysis) { // Check for data.results.analysis
        console.warn("Assessment data not found, results missing, or analysis missing");
        setResults(getDefaultResults());
        return;
      }
      setResults(data.results); // Assuming data.results contains the { analysis: { ... } } structure
  ...
  const handleTakeNewAssessment = async () => {
    try {
      ...
      setResults(null); // Clear results from context
      navigate("/assessment"); // Use navigate
    } catch (error) {
  ...
  useEffect(() => {
    if (isLoaded) { // Fetch whether user exists or not to handle guest state
        fetchResults();
    }
  }, [isLoaded, user]); // Rerun if user object changes (e.g., after login)
  ...
  // if (isLoading) {
  //   return <div className="dashboard-container loading-container"><p>Loading your dashboard...</p></div>;
  // }

  if (!user && isLoaded) { // If clerk is loaded but there's no user
    return (
  ...
  // Check if results or essential parts of results are missing after loading and user is present
        ...
        <div className="welcome-content">
          <div className="welcome-header">
            <div className="welcome-text">
              ...
              <p>You haven't taken the assessment yet, or your results are not available.</p>
            </div>
            {user && (
          ...
          <div className="welcome-card">
            ...
            <p>
              Take our comprehensive career assessment to unlock personalized insights,
              career recommendations, and a roadmap for your professional growth.
            </p>
  ...
  const renderSectionContent = () => {
    if (!results || !results.analysis) return <p>No assessment data available.</p>; // Should be caught by above
    ...
    switch (activeSection) {
                  ...
                  <div className="opportunity-section">
                    ...
                    <ul className="recommendation-list">
                      {analysis.growthOpportunities.sectors.map((sector, index) => (
                        <li key={index} className="sector-item">{sector}</li>
                      ))}
                    </ul>
                  </div>
                </div>
                  ...
                  <div className="steps-section">
                    ...
                    <ul className="recommendation-list">
                      {analysis.actionPlan.immediateNextSteps.map((step, index) => (
                        <li key={index} className="step-item"><p>{step}</p></li>
                      ))}
                    </ul>
                  </div>
                  <div className="steps-section">
                    <h4>Short Term Goals</h4>
                     <ul className="recommendation-list">
                      {analysis.actionPlan.shortTermGoals.map((goal, index) => (
                        <li key={index} className="goal-item"><p>{goal}</p></li>
                      ))}
                    </ul>
                  </div>
                  <div className="steps-section">
                    ...
                    <ul className="recommendation-list">
                      {analysis.actionPlan.longTermRoadmap.map((roadmap, index) => (
                        <li key={index} className="roadmap-item"><p>{roadmap}</p></li>
                      ))}
                    </ul>
                  </div>
                </div>
                      ...
                      <div key={index} className="detailed-path-item">
                        ...
                        <div className="path-details">
                          ...
                          <div className="path-actions">
                            {/* Link to a future career details page if you have one */}
                            <Link to={`/explore/career/${slugify(recommendation.jobTitle)}`} className="path-action-button">
      ...
      case "schools":
        return (
          <div className="section-content">
            <h2 className="section-title">Schools & Education</h2>
            <div className="dashboard-grid">
              <ExpandableCard
                id="schools"
                title="Recommended Schools"
                icon={<GraduationCap className="card-icon-svg" />}
                content={
                  <div className="schools-section">
                    <p>Explore top educational institutions that offer programs aligned with your career goals.</p>
                    <div className="school-recommendations">
                      {(analysis.careerRecommendations || []).slice(0, 3).map((career, index) => (
                        <div key={index} className="school-recommendation-item">
                          <h4>{career.jobTitle} Programs</h4>
                          <p>Find schools offering specialized education in {career.jobTitle.toLowerCase()}.</p>
                          <Link to={`/schools/career/${slugify(career.jobTitle)}`} className="school-explore-button">
                              Find Schools <ArrowRight size={16} />
                          </Link>
                        </div>
                      ))}
                    </div>
                    {(analysis.recommendedSchools && analysis.recommendedSchools.length > 0) && (
                      <div className="view-all-schools">
                        <Link to="/schools/all" className="view-all-button">
                            View All Recommended Schools <ArrowRight size={16} />
                        </Link>
                      </div>
                    )}
                  </div>
      ...
      case "mentors":
        return (
          <div className="section-content">
            <h2 className="section-title">Mentors & Guidance</h2>
            <div className="dashboard-grid">
              <ExpandableCard
                id="mentors"
                title="Connect with Mentors"
                icon={<UserCheck className="card-icon-svg" />}
                content={
                  <div className="mentors-section">
                    <p>Connect with experienced professionals who can guide you on your career journey.</p>
                    <div className="mentor-recommendations">
                      {(analysis.careerRecommendations || []).slice(0, 3).map((career, index) => (
                        <div key={index} className="mentor-recommendation-item">
                          <h4>{career.jobTitle} Mentors</h4>
                          <p>Find mentors with expertise in {career.jobTitle.toLowerCase()}.</p>
                           <Link to={`/mentors/career/${slugify(career.jobTitle)}`} className="mentor-explore-button">
                      ...
                      <div className="view-all-mentors">
                        <Link to="/mentors/all" className="view-all-button">
                            View All Recommended Mentors <ArrowRight size={16} />
                        </Link>
                      </div>
     ...
     case "development":
    // This line is still correct and is a good way to safely get the data.
    const { recommendedCourses = [], suggestedReadings = [], professionalTools = [] } = analysis.resources || {};
    
    return (
        <div className="section-content">
        <h2 className="section-title">Professional Development Resources</h2>
        <div className="dashboard-grid-large">
            ...
            <ExpandableCard
                ...
                <div className="resource-list">
                    {/* --- CHANGE IS HERE --- */}
                    {/* We map over the array of strings. 'course' is now the string itself. */}
                    {recommendedCourses.map((course, index) => (
                    <div key={`course-${index}`} className="resource-item-card">
                        {/* We display the 'course' string directly */}
                        <h4>{course}</h4>
                    </div>
                    ))}
                </div>
            ...
            <ExpandableCard
                ...
                <div className="resource-list">
                    {/* --- CHANGE IS HERE --- */}
                    {/* 'reading' is the string from the array */}
                    {suggestedReadings.map((reading, index) => (
                    <div key={`reading-${index}`} className="resource-item-card">
                        {/* Display the 'reading' string */}
                        <h4>{reading}</h4>
                    </div>
                    ))}
                </div>
            ...
            <ExpandableCard
                ...
                <div className="resource-list">
                    {/* --- CHANGE IS HERE --- */}
                    {/* 'tool' is the string from the array */}
                    {professionalTools.map((tool, index) => (
                    <div key={`tool-${index}`} className="resource-item-card">
                        {/* Display the 'tool' string */}
                        <h4>{tool}</h4>
                    </div>
                    ))}
                </div>
            ...
            {(recommendedCourses.length === 0 && suggestedReadings.length === 0 && professionalTools.length === 0) && (
            <p>No specific development resources recommended at this time. Explore general learning platforms!</p>
            )}
        </div>
        </div>
    );
  ...
  return (
    <div className="dashboard-container">
      ...
      <nav className="dashboard-nav">
        {["overview", "skills", "paths", "schools", "mentors", "development"].map((section) => (
          <button
            key={section}
